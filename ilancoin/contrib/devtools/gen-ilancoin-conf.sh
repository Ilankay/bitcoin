...
EXAMPLE_CONF_FILE=${EXAMPLE_CONF_FILE:-$SHARE_EXAMPLES_DIR/ilancoin.conf}

[ ! -x "$ILANCOIND" ] && echo "$ILANCOIND not found or not executable." && exit 1

DIRTY=""
VERSION_OUTPUT=$($ILANCOIND --version)
if [[ $VERSION_OUTPUT == *"dirty"* ]]; then
  DIRTY="${DIRTY}${ILANCOIND}\n"
fi

if [ -n "$DIRTY" ]
then
  echo -e "WARNING: $ILANCOIND was built from a dirty tree.\n"
  echo -e "To safely generate a ilancoin.conf file, please commit your changes to $ILANCOIND, rebuild, then run this script again.\n"
fi

echo 'Generating example ilancoin.conf file in share/examples/'
...
## Generated by contrib/devtools/gen-ilancoin-conf.sh.
##
...
# parse the output from ilancoind --help
