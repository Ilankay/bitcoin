...
Tracing (USDT) support for Ilancoin Core on Linux using. For more information on
USDT support in Ilancoin Core see the [USDT documentation].
...
The bpftrace examples contain a relative path to the `ilancoind` binary. By
default, the scripts should be run from the repository-root and assume a
self-compiled `ilancoind` binary. The paths in the examples can be changed, for
example, to point to release builds if needed. See the
[Ilancoin Core USDT documentation] on how to list available tracepoints in your
`ilancoind` binary.

[Ilancoin Core USDT documentation]: ../../doc/tracing.md#listing-available-tracepoints
...
```
$ python3 contrib/tracing/p2p_monitor.py $(pidof ilancoind)
```
...
Ilancoin P2P messages can be larger than 32kb (e.g. `tx`, `block`, ...). The
...
```
$ python3 contrib/tracing/log_raw_p2p_msgs.py $(pidof ilancoind)
```
...
In a different terminal, starting Ilancoin Core in SigNet mode and with
re-indexing enabled.

```
$ ./build/src/ilancoind -signet -reindex
```
...
```bash
$ python3 contrib/tracing/log_utxocache_flush.py $(pidof ilancoind)
```
...
```console
$ python3 contrib/tracing/mempool_monitor.py $(pidof ilancoind)
```
