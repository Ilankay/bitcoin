...
""" Example logging Ilancoin Core mempool events using the mempool:added,
...
def main(pid):
    print(f"Hooking into ilancoind with pid {pid}")
    ilancoind_with_usdts = USDT(pid=int(pid))

    # attaching the trace functions defined in the BPF program
    # to the tracepoints
    ilancoind_with_usdts.enable_probe(probe="mempool:added", fn_name="trace_added")
    ilancoind_with_usdts.enable_probe(probe="mempool:removed", fn_name="trace_removed")
    ilancoind_with_usdts.enable_probe(probe="mempool:replaced", fn_name="trace_replaced")
    ilancoind_with_usdts.enable_probe(probe="mempool:rejected", fn_name="trace_rejected")
    bpf = BPF(text=PROGRAM, usdt_contexts=[ilancoind_with_usdts])
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("USAGE: ", sys.argv[0], "<pid of ilancoind>")
        exit(1)
