...
static void SetupIlancoinTxArgs(ArgsManager &argsman)
{
...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs) || gArgs.IsArgSet("-version")) {
        ...
        std::string strUsage = CLIENT_NAME " ilancoin-tx utility version " + FormatFullVersion() + "\n";

        if (gArgs.IsArgSet("-version")) {
            strUsage += FormatParagraph(LicenseInfo());
        } else {
            strUsage += "\n"
                "The ilancoin-tx tool is used for creating and modifying ilancoin transactions.\n\n"
                "ilancoin-tx can be used with \"<hex-tx> [commands]\" to update a hex-encoded ilancoin transaction, or with \"-create [commands]\" to create a hex-encoded ilancoin transaction.\n"
                "\n"
                "Usage: ilancoin-tx [options] <hex-tx> [commands]\n"
                "or:    ilancoin-tx [options] -create [commands]\n"
                "\n";
