...
/** Call node.updateRwSetting() with Ilancoin 22.x workaround. */
static void UpdateRwSetting(interfaces::Node& node, OptionsModel::OptionID option, const std::string& suffix, const common::SettingsValue& value)
{
    if (value.isNum() &&
        ...
        // because Ilancoin 22.x releases try to read these specific settings as
        ...
        // in later releases by https://github.com/ilancoin/ilancoin/pull/24498.
        // If new numeric settings are added, they can be written as numbers
        // instead of strings, because ilancoin 22.x will not try to read these.
...
//! Convert enabled/size values to ilancoin -prune setting.
...
//! Get pruning enabled value to show in GUI from ilancoin -prune setting.
...
//! Get pruning size value to show in GUI from ilancoin -prune setting. If
...
bool OptionsModel::Init(bilingual_str& error)
{
    ...
    // Display
    if (!settings.contains("DisplayIlancoinUnit")) {
        settings.setValue("DisplayIlancoinUnit", QVariant::fromValue(IlancoinUnit::ITC));
    }
    QVariant unit = settings.value("DisplayIlancoinUnit");
    if (unit.canConvert<IlancoinUnit>()) {
        m_display_ilancoin_unit = unit.value<IlancoinUnit>();
    } else {
        m_display_ilancoin_unit = IlancoinUnit::ITC;
        settings.setValue("DisplayIlancoinUnit", QVariant::fromValue(m_display_ilancoin_unit));
    }
...
void OptionsModel::SetPruneTargetGB(int prune_target_gb)
{
    ...
    // from saved value. Avoid writing settings.json if ilancoin.conf value
...
#endif
    case DisplayUnit:
        return QVariant::fromValue(m_display_ilancoin_unit);
...
{
    if (new_unit.isNull() || new_unit.value<IlancoinUnit>() == m_display_ilancoin_unit) return;
    m_display_ilancoin_unit = new_unit.value<IlancoinUnit>();
    QSettings settings;
    settings.setValue("DisplayIlancoinUnit", QVariant::fromValue(m_display_ilancoin_unit));
    Q_EMIT displayUnitChanged(m_display_ilancoin_unit);
}
...
void OptionsModel::checkAndMigrate()
{
    ...
    // (https://github.com/ilancoin-core/gui/issues/567).
