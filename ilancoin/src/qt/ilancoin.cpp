...
static void RegisterMetaTypes()
{
    ...
    qRegisterMetaTypeStreamOperators<IlancoinUnit>("IlancoinUnit");
#else
    qRegisterMetaType<IlancoinUnit>("IlancoinUnit");
#endif
}
...
bool IlancoinApplication::createOptionsModel(bool resetSettings)
{
...
{
    window = new IlancoinGUI(node(), platformStyle, networkStyle, nullptr);
    connect(window, &IlancoinGUI::quitRequested, this, &IlancoinApplication::requestShutdown);
...
void IlancoinApplication::createNode(interfaces::Init& init)
{
    assert(!m_node);
...
void IlancoinApplication::startThread()
{
    ...
    connect(&m_executor.value(), &InitExecutor::initializeResult, this, &IlancoinApplication::initializeResult);
    connect(&m_executor.value(), &InitExecutor::shutdownResult, this, [] {
        QCoreApplication::exit(0);
    });
    connect(&m_executor.value(), &InitExecutor::runawayException, this, &IlancoinApplication::handleRunawayException);
    connect(this, &IlancoinApplication::requestedInitialize, &m_executor.value(), &InitExecutor::initialize);
    connect(this, &IlancoinApplication::requestedShutdown, &m_executor.value(), &InitExecutor::shutdown);
}
...
void IlancoinApplication::InitPruneSetting(int64_t prune_MiB)
{
...
void IlancoinApplication::initializeResult(bool success, interfaces::BlockAndHeaderTipInfo tip_info)
{
...
void IlancoinApplication::handleNonFatalException(const QString& message)
{
...
bool IlancoinApplication::event(QEvent* e)
{
...
static void SetupUIArgs(ArgsManager& argsman)
{
    ...
    argsman.AddArg("-uiplatform", strprintf("Select platform to customize UI for (one of windows, macosx, other; default: %s)", IlancoinGUI::DEFAULT_UIPLATFORM), ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::GUI);
}
...
int GuiMain(int argc, char* argv[])
{
    ...
    for (int i = 1; i < argc; i++) {
...
#ifdef ENABLE_WALLET
        if (arg.startsWith(ILANCOIN_IPC_PREFIX, Qt::CaseInsensitive)) {
            invalid_token &= false;
        ...
        if (invalid_token) {
            InitError(Untranslated(strprintf("Command line contains unexpected token '%s', see ilancoin-qt -h for a list of options.", argv[i])));
            QMessageBox::critical(nullptr, CLIENT_NAME,
                                  // message cannot be translated because translations have not been initialized
                                  QString::fromStdString("Command line contains unexpected token '%1', see ilancoin-qt -h for a list of options.").arg(QString::fromStdString(argv[i])));
            return EXIT_FAILURE;
        }
    }
    ...
    /// 6-7. Parse ilancoin.conf, determine network, switch to network specific
