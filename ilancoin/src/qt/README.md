This directory contains the source code for the Ilancoin Core graphical user interface (GUI). It uses the [Qt](https://www1.qt.io/developers/) cross-platform framework.
...
- (Deprecated) Used to process BIP21 payment URI requests. Also handles URI-based application switching (e.g. when following a ilancoin:... link from a browser).
...
* `ilancoinunits.(h/cpp)`: ITC / mITC / etc. handling
* `callback.h`
...
**Note:** Do not change `local/ilancoin_en.ts`. It is updated [automatically](/doc/translation_process.md#writing-code-with-translations).
...
4. Enter `ilancoin-qt` as the Project Name and enter the absolute path to `src/qt` as Location
...
8. While in the `Projects` tab, ensure that you have the `ilancoin-qt` executable specified under `Run`
 - If the executable is not specified: click `"Choose..."`, navigate to `src/qt`, and select `ilancoin-qt`
9. You're all set! Start developing, building, and debugging the Ilancoin Core GUI
