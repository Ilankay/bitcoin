...
{
    fs::path settings = GetPathArg("-settings", ILANCOIN_SETTINGS_FILENAME);
    if (settings.empty()) {
        return false;
    }
    if (backup) {
...
{
    // Windows:
    //   old: C:\Users\Username\AppData\Roaming\Ilancoin
    //   new: C:\Users\Username\AppData\Local\Ilancoin
    // macOS: ~/Library/Application Support/Ilancoin
    // Unix-like: ~/.ilancoin
#ifdef WIN32
    // Windows
    // Check for existence of datadir in old location and keep it there
    fs::path legacy_path = GetSpecialFolderPath(CSIDL_APPDATA) / "Ilancoin";
    if (fs::exists(legacy_path)) return legacy_path;

    // Otherwise, fresh installs can start in the new, "proper" location
    return GetSpecialFolderPath(CSIDL_LOCAL_APPDATA) / "Ilancoin";
#else
    fs::path pathRet;
