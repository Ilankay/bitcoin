// Copyright (c) 2016-present The Ilancoin Core developers
...
static std::optional<int> WalletAppInit(ArgsManager& args, int argc, char* argv[])
{
    ...
    if (missing_args || HelpRequested(args) || args.IsArgSet("-version")) {
        std::string strUsage = strprintf("%s ilancoin-wallet utility version", CLIENT_NAME) + " " + FormatFullVersion() + "\n";

        if (args.IsArgSet("-version")) {
            strUsage += FormatParagraph(LicenseInfo());
        } else {
            strUsage += "\n"
                "ilancoin-wallet is an offline tool for creating and interacting with " CLIENT_NAME " wallet files.\n\n"
                ...
                "\n"
                "Usage: ilancoin-wallet [options] <command>\n"
                "\n";
            strUsage += "\n" + args.GetHelpMessage();
        }
