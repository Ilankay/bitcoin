// Copyright (c) The Ilancoin Core developers
...
fn get_linter_list() -> Vec<&'static Linter> {
    vec![
        ...
        &Linter {
            description: "Check that no symbol from ilancoin-build-config.h is used without the header being included",
...
fn lint_includes_build_config() -> LintResult {
    let config_path = "./cmake/ilancoin-build-config.h.in";
    ...
    let print_affected_files = |mode: bool| {
        ...
        let defines_files = check_output(
            git()
                ...
                .args(get_pathspecs_exclude_subtrees())
                .args([
                    // These are exceptions which don't use ilancoin-build-config.h, rather CMakeLists.txt adds
        ...
        git()
            .args([
                ...
                if mode {
                    ...
                    "#include <ilancoin-build-config.h>" // Catch redundant includes with and without the IWYU pragma
                },
                "--",
            ])
    ...
    if missing {
        return Err(format!(
            r#"
^^^
One or more files use a symbol declared in the ilancoin-build-config.h header. However, they are not
...
even though ilancoin-build-config.h indicates that a faster feature is available and should be used.
...
#include <ilancoin-build-config.h> // IWYU pragma: keep
    ...
    if redundant {
        return Err(r#"
^^^
None of the files use a symbol declared in the ilancoin-build-config.h header. However, they are including
