...
from test_framework.test_node import (
    ILANCOIN_PID_FILENAME_DEFAULT,
    ErrorMatch,
)
from test_framework.util import assert_equal


class InitTest(IlancoinTestFramework):
    """
    ...
    def init_pid_test(self):
        ILANCOIN_PID_FILENAME_CUSTOM = "my_fancy_ilancoin_pid_file.foobar"

        self.log.info("Test specifying custom pid file via -pid command line option")
        custom_pidfile_relative = ILANCOIN_PID_FILENAME_CUSTOM
        ...
        assert not (datadir / ILANCOIN_PID_FILENAME_DEFAULT).exists()
        assert (datadir / custom_pidfile_relative).exists()
        self.stop_node(0)
        assert not (datadir / custom_pidfile_relative).exists()

        custom_pidfile_absolute = Path(self.options.tmpdir) / ILANCOIN_PID_FILENAME_CUSTOM
