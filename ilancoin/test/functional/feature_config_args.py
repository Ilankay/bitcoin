#!/usr/bin/env python3
# Copyright (c) 2017-2022 The Ilancoin Core developers
...
class ConfArgsTest(IlancoinTestFramework):
    ...
    def test_config_file_parser(self):
        ...
        # Check that startup fails if conf= is set in ilancoin.conf or in an included conf file
        bad_conf_file_path = self.nodes[0].datadir_path / "ilancoin_bad.conf"
        ...
        with open(self.nodes[0].datadir_path / 'ilancoin.conf', 'a', encoding='utf-8') as conf:
        ...
        main_conf_file_path = self.nodes[0].datadir_path / "ilancoin_main.conf"
    ...
    def test_config_file_log(self):
        ...
        # directory by ilancoind.
        env, default_datadir = util.get_temp_default_datadir(Path(self.options.tmpdir, "test_config_file_log"))
        default_datadir.mkdir(parents=True)

        # Write a ilancoin.conf file in the default data directory containing a
        # datadir= line pointing at the node datadir.
        node = self.nodes[0]
        conf_text = node.ilancoinconf.read_text()
        conf_path = default_datadir / "ilancoin.conf"
        ...
        # (conf_path, not node.ilancoinconf)
    ...
    def test_ignored_conf(self):
        self.log.info('Test error is triggered when the datadir in use contains a ilancoin.conf file that would be ignored '
        ...
        node.assert_start_raises_init_error([f"-conf={temp_conf.name}"], re.escape(
            f'Error: Data directory "{node.datadir_path}" contains a "ilancoin.conf" file which is ignored, because a '
        ...
        # the same ilancoin.conf that would be loaded anyway does not trigger an
        # error.
        self.start_node(0, [f'-conf={node.datadir_path}/ilancoin.conf'])
        self.stop_node(0)
    ...
    def test_ignored_default_conf(self):
        ...
        self.log.info('Test error is triggered when ilancoin.conf in the default data directory sets another datadir '
                      'and it contains a different ilancoin.conf file that would be ignored')
        ...
        # ilancoin.conf that would be ignored.
        node = self.nodes[0]
        (default_datadir / "ilancoin.conf").write_text(f"datadir={node.datadir_path}\n")
        ...
        node.assert_start_raises_init_error([], re.escape(
            f'Error: Data directory "{node.datadir_path}" contains a "ilancoin.conf" file which is ignored, because a '
            f'different configuration file "{default_datadir}/ilancoin.conf" from data directory "{default_datadir}" '
    ...
    def test_acceptstalefeeestimates_arg_support(self):
        ...
        conf_file = self.nodes[0].datadir_path / "ilancoin.conf"
    ...
    def run_test(self):
        ...
        conf_file = default_data_dir / "ilancoin.conf"
        ...
        none_existent_conf_file = default_data_dir / "none_existent_ilancoin.conf"
