#!/usr/bin/env python3
# Copyright (c) 2014-present The Ilancoin Core developers
...
class IlancoinTestFramework(metaclass=IlancoinTestMetaClass):
    ...
    def set_binary_paths(self):
        ...
        binaries = {
            "ilancoind": ("ilancoind", "ILANCOIND"),
            "ilancoin-cli": ("ilancoincli", "ILANCOINCLI"),
            "ilancoin-util": ("ilancoinutil", "ILANCOINUTIL"),
            "ilancoin-wallet": ("ilancoinwallet", "ILANCOINWALLET"),
        }
    ...
    def create_outpoints(self, node, *, outputs):
        """Send funds to a given list of `{address: amount}` targets using the ilancoind
    ...
    def _initialize_chain(self):
        ...
        for i in range(self.num_nodes):
            ...
            initialize_datadir(self.options.tmpdir, i, self.chain, self.disable_autoconnect)  # Overwrite port/rpcport in ilancoin.conf
    ...
    def skip_if_no_ilancoind_tracepoints(self):
        """Skip the running test if ilancoind has not been compiled with USDT tracepoint support."""
        if not self.is_usdt_compiled():
            raise SkipTest("ilancoind has not been built with USDT tracepoints enabled.")
    ...
    def skip_if_no_ilancoin_util(self):
        """Skip the running test if ilancoin-util has not been compiled."""
        if not self.is_ilancoin_util_compiled():
            raise SkipTest("ilancoin-util has not been compiled")
    ...
    def is_ilancoin_util_compiled(self):
        """Checks whether ilancoin-util was compiled."""
        return self.config["components"].getboolean("ENABLE_ILANCOIN_UTIL")
