...
ILANCOIN_PID_FILENAME_DEFAULT = "ilancoind.pid"
...
class TestNode():
    ...
    def __init__(self, i, datadir_path, *, chain, rpchost, timewait, timeout_factor, ilancoind, ilancoin_cli, coverage_dir, cwd, extra_conf=None, extra_args=None, use_cli=False, start_perf=False, use_valgrind=False, version=None, descriptors=False, v2transport=False):
        """
        Kwargs:
        ...
        self.ilancoinconf = self.datadir_path / "ilancoin.conf"
        ...
        self.cli = TestNodeCLI(ilancoin_cli, self.datadir_path)
    ...
    def start(self, extra_args=None, *, cwd=None, stdout=None, stderr=None, env=None, **kwargs):
        ...
        # If listening and no -bind is given, then ilancoind would bind P2P ports on
        ...
        # ilancoin.conf. To avoid collisions on 127.0.0.1:18445, change it to
        # 127.0.0.1:tor_port().
        ...
        for _ in range(poll_per_s * self.rpc_timeout):
            if self.process.poll() is not None:
                ...
                raise FailedToStartError(self._node_msg(
                    f'ilancoind exited with status {self.process.returncode} during initialization. {str_error}'))
            try:
                rpc = get_rpc_proxy(
    ...
    def wait_until_stopped(self, *, timeout=ILANCOIND_PROC_WAIT_TIMEOUT, expect_error=False, **kwargs):
