...
def assert_fee_amount(fee, tx_size, feerate_ITC_kvB):
    """Assert the fee is in range."""
    assert isinstance(tx_size, int)
    target_fee = get_fee(tx_size, feerate_ITC_kvB)
    if fee < target_fee:
        raise AssertionError("Fee of %s ITC too low! (Should be %s ITC)" % (str(fee), str(target_fee)))
    # allow the wallet's estimation to be at most 2 bytes off
    high_fee = get_fee(tx_size + 2, feerate_ITC_kvB)
    if fee > high_fee:
...
def get_fee(tx_size, feerate_btc_kvb):
    """Calculate the fee in ITC given a feerate is ITC/kvB. Reflects CFeeRate::GetFee"""
...
def wait_until_helper_internal(predicate, *, timeout=60, lock=None, timeout_factor=1.0, check_interval=0.05):
    ...
    from `IlancoinTestFramework` or `P2PInterface` class ensures the timeout is
...
def initialize_datadir(dirname, n, chain, disable_autoconnect=True):
    ...
    if not os.path.isdir(datadir):
        os.makedirs(datadir)
    write_config(os.path.join(datadir, "ilancoin.conf"), n=n, chain=chain, disable_autoconnect=disable_autoconnect)
...
def get_temp_default_datadir(temp_dir: pathlib.Path) -> tuple[dict, pathlib.Path]:
    ...
    if platform.system() == "Windows":
        ...
        datadir = temp_dir / "Ilancoin"
    else:
        env = dict(HOME=str(temp_dir))
        if platform.system() == "Darwin":
            datadir = temp_dir / "Library/Application Support/Ilancoin"
        else:
            datadir = temp_dir / ".ilancoin"
    return env, datadir
