#!/usr/bin/env python3
# Copyright (c) 2014-2022 The Ilancoin Core developers
...
class RawTransactionsTest(IlancoinTestFramework):
    ...
    def test_transaction_too_large(self):
        ...
        # Make 1500 0.1 ITC outputs. The amount that we target for funding is in
    ...
    def test_add_inputs_default_value(self):
        ...
        # Create and fund the wallet with 5 ITC
        ...
        # Fund wallet with 2 outputs, 5 ITC each.
    ...
    def test_preset_inputs_selection(self):
        ...
        # Create and fund the wallet with 4 UTXO of 5 ITC each (20 ITC total)
        ...
        # Attempt to send 29 ITC from a wallet that only has 20 ITC. The wallet should exclude
        # the preset inputs from the pool of available coins, realize that there is not enough
        # money to fund the 29 ITC payment, and fail with "Insufficient funds".
        #
        # Even with SFFO, the wallet can only afford to send 20 ITC.
        # If the wallet does not properly exclude preset inputs from the pool of available coins
        # prior to coin selection, it may create a transaction that does not fund the full payment
        # amount or, through SFFO, incorrectly reduce the recipient's amount by the difference
        # between the original target and the wrongly counted inputs (in this case 9 ITC)
        # so that the recipient's amount is no longer equal to the user's selected target of 29 ITC.
    ...
    def test_22670(self):
        ...
        # that this bug is not detected because the transaction fee must be at least 0.01 ITC (the minimum change value).
        # Otherwise the targeted minimum change value will be enough to cover the transaction fees that were not
        # being accounted for. So the minimum relay fee is set to 0.1 ITC/kvB in this test.
    ...
    def test_feerate_rounding(self):
        ...
        # fail with insufficient funds rather than ilancoind asserting.
    ...
    def test_input_confs_control(self):
        ...
        # Create a replacement tx to 'final_tx1' that has 1 ITC target instead of 0.1.
        ...
        # Now fund 'raw_tx2' to fulfill the total target (1 ITC) by using all the wallet unconfirmed outputs.
        # As it was created with the first unconfirmed output, 'raw_tx2' only has 0.1 ITC covered (need to fund 0.9 ITC more).
