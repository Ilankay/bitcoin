# CJDNS support in Ilancoin Core

It is possible to run Ilancoin Core over CJDNS, an encrypted IPv6 network that
...
done outside of Ilancoin Core, similarly to a VPN (either in the host/OS or on
...
redundancy and robustness for both the Ilancoin network and individual nodes.
...
before it will work with your Ilancoin Core node. This is described in steps
...
## Run Ilancoin Core with CJDNS

Once you are connected to the CJDNS network, the following Ilancoin Core
...
When enabled, this option tells Ilancoin Core that it is running in an
...
IPv6 local network. This helps Ilancoin Core perform better address management:
...
CJDNS support was added to Ilancoin Core in version 23.0 and there may be fewer
CJDNS peers than Tor or IP ones. You can use `ilancoin-cli -addrinfo` to see the
...
[doc/tor.md](https://github.com/ilancoin/ilancoin/blob/master/doc/tor.md) for
details.

## CJDNS-related information in Ilancoin Core

There are several ways to see your CJDNS address in Ilancoin Core:
- in the "Local addresses" output of CLI `-netinfo`
- in the "localaddresses" output of RPC `getnetworkinfo`

To see which CJDNS peers your node is connected to, use `ilancoin-cli -netinfo 4`
or the `getpeerinfo` RPC (i.e. `ilancoin-cli getpeerinfo`).

You can use the `getnodeaddresses` RPC to fetch a number of CJDNS peers known to your node; run `ilancoin-cli help getnodeaddresses` for details.
