...
### 2. Clone Ilancoin Repo
Now that `git` and all the required dependencies are installed, let's clone the Ilancoin Core repository to a directory. All build scripts and commands will run from this directory.
```bash
...
#### Wallet Dependencies
It is not necessary to build wallet functionality to run either `ilancoind` or `ilancoin-qt`.
...
```
to: /path/to/ilancoin/depends/x86_64-unknown-freebsd[release-number]
```
...
Ilancoin Core can provide notifications via ZeroMQ. If the package is installed, support will be compiled in.
```bash
pkg install libzmq4
```
