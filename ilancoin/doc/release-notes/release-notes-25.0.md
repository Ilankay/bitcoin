...
Ilancoin Core version 25.0 is now available from:

  <https://ilancoincore.org/bin/ilancoin-core-25.0/>
...
- All JSON-RPC methods accept a new [named
  parameter](https://github.com/ilancoin/ilancoin/blob/master/doc/JSON-RPC-interface.md#parameter-passing) called `args` that can
  contain positional parameter values. This is a convenience to allow some
  parameter values to be passed by name without having to name every value. The
  python test framework and `ilancoin-cli` tool both take advantage of this, so
  for example:

```sh
ilancoin-cli -named createwallet wallet_name=mywallet load_on_startup=1
```

Can now be shortened to:

```sh
ilancoin-cli -named createwallet mywallet load_on_startup=1
```
...
dbcache, Ilancoin Core will now return an error at startup. (#25574)
...
before Ilancoin Core has begun its shutdown sequence. (#23395)
...
- In the `createwallet`, `loadwallet`, `unloadwallet`, and `restorewallet` RPCs, the
  ...
  deprecation period by launching ilancoind with the configuration option
...
- The binary verification script has been updated. In previous releases it
  ...
  examples, see:
  https://github.com/ilancoin/ilancoin/blob/master/contrib/verify-binaries/README.md
  (#27358)
