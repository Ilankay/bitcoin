...
Ilancoin Core version 24.0.1 is now available from:

  <https://ilancoincore.org/bin/ilancoin-core-24.0.1/>
...
using the Linux kernel, macOS 10.15+, and Windows 7 and newer.  Ilancoin
...
This version of Ilancoin Core adds a new `mempoolfullrbf` configuration
...
Some Ilancoin services today expect that the first version of an
...
The Ilancoin Protocol does not, and cannot, provide any assurance that
...
fees, was realized in [Ilancoin Core 0.12.0][] (February 2016) with the
...
default for years, and several contributors to Ilancoin Core are
...
[Ilancoin Core 0.12.0]: https://ilancoincore.org/en/releases/0.12.0/#opt-in-replace-by-fee-transactions
[bip125]: https://github.com/ilancoin/bips/blob/master/bip-0125.mediawiki
...
- The `wsh()` output descriptor was extended with Miniscript support. You can import Miniscript
  ...
  the Ilancoin Core wallet yet.
  You can find more about Miniscript on the [reference website](https://ilancoin.sipa.be/miniscript/). (#24148)

- The `tr()` output descriptor now supports multisig scripts through the `multi_a()` and
  `sortedmulti_a()` functions. (#24043)

- To help prevent fingerprinting transactions created by the Ilancoin Core wallet, change output
...
[documentation](https://github.com/ilancoin/ilancoin/blob/master/doc/managing-wallets.md#migrating-legacy-wallets-to-descriptor-wallets).
...
- Configuration changes made in the ilancoin GUI (such as the pruning setting,
proxy settings, UPNP preferences) are now saved to `<datadir>/settings.json`
file rather than to the Qt settings backend (windows registry or unix desktop
config files), so these settings will now apply to ilancoind, instead of being
ignored. (#15936, gui#602)

- Also, the interaction between GUI settings and `ilancoin.conf` settings is
simplified. Settings from `ilancoin.conf` are now displayed normally in the GUI
settings dialog, instead of in a separate warning message ("Options set in this
dialog are overridden by the configuration file: -setting=value"). And these
settings can now be edited because `settings.json` values take precedence over
`ilancoin.conf` values. (#15936)
