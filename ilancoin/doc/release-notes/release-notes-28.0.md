Ilancoin Core version 28.0 is now available from:

  <https://ilancoincore.org/bin/ilancoin-core-28.0/>
...
installer (on Windows) or just copy over `/Applications/Ilancoin-Qt` (on macOS)
...
Running Ilancoin Core binaries on macOS requires self signing.
```
cd /path/to/ilancoin-28.0/bin
xattr -d com.apple.quarantine ilancoin-cli ilancoin-qt ilancoin-tx ilancoin-util ilancoin-wallet ilancoind test_ilancoin
codesign -s - ilancoin-cli ilancoin-qt ilancoin-tx ilancoin-util ilancoin-wallet ilancoind test_ilancoin
```
...
using the Linux Kernel 3.17+, macOS 11.0+, and Windows 7 and newer. Ilancoin
...
Support for Testnet4 as specified in [BIP94](https://github.com/ilancoin/bips/blob/master/bip-0094.mediawiki)
...
The default data directory on Windows has been moved from `C:\Users\Username\AppData\Roaming\Ilancoin`
to `C:\Users\Username\AppData\Local\Ilancoin`. Ilancoin Core will check the existence
...
See [JSON-RPC-interface.md](https://github.com/ilancoin/ilancoin/blob/master/doc/JSON-RPC-interface.md#json-rpc-11-vs-20) for details. (#27101)
...
libilancoinconsensus Removal
---------------------------

The libilancoin-consensus library was deprecated in 27.0 and is now completely removed. (#29648)

P2P and Network Changes
-----------------------

- Previously if Ilancoin Core was listening for P2P connections, either using
...
- Ilancoin Core will now fail to start up if any of its P2P binds fail, rather
...
- Transactions with version number set to 3 are now treated as standard on all networks (#29496),
  ...
  described in [BIP 431](https://github.com/ilancoin/bips/blob/master/bip-0431.mediawiki).  The
...
- The `warnings` field in `getblockchaininfo`, `getmininginfo` and
  ...
  can be temporarily restored by running Ilancoin Core with the configuration
...
are now required to compile Ilancoin Core. (#29091, #30263)

- The minimum required glibc to run Ilancoin Core is now
...
- A new `-testdatadir` option has been added to `test_ilancoin` to allow specifying the
...
Previous releases of Ilancoin Core or previous external software will not be able to read the blocksdir with a non-zero XOR-key.
