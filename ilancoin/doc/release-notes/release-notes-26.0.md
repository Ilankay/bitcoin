...
Ilancoin Core version 26.0 is now available from:

  <https://ilancoincore.org/bin/ilancoin-core-26.0/>
...
- Experimental support for the v2 transport protocol defined in
  [BIP324](https://github.com/ilancoin/bips/blob/master/bip-0324.mediawiki) was added.
...
- [Miniscript](https://ilancoin.sipa.be/miniscript/) expressions can now be used in Taproot descriptors for all RPCs working with descriptors. (#27255)

- `finalizepsbt` is now able to finalize a PSBT with inputs spending [Miniscript](https://ilancoin.sipa.be/miniscript/)-compatible Taproot leaves. (#27255)
...
- `ilancoind` and `ilancoin-qt` will now raise an error on startup
 if a datadir that is being used contains a ilancoin.conf file that
 will be ignored, which can happen when a datadir= line is used in
 a ilancoin.conf file. The error message is just a diagnostic intended
 ...
 the ilancoin.conf contained in it. (#27302)
...
- A new `ilancoinconsensus_verify_script_with_spent_outputs` function is available in libconsensus which optionally accepts the spent outputs of the transaction being verified.
- A new `ilancoinconsensus_SCRIPT_FLAGS_VERIFY_TAPROOT` flag is available in libconsensus that will verify scripts with the Taproot spending rules.
...
- Wallet loading has changed in this release. Wallets with some corrupted records that could be
  ...
  load the wallet in a previous version of Ilancoin Core and import the data into a new wallet.
...
```sh
src/ilancoin-cli -named bumpfee txid fee_rate=100
```

instead of

```sh
src/ilancoin-cli -named bumpfee txid options='{"fee_rate": 100}'
```
...
- It's now possible to use [Miniscript](https://ilancoin.sipa.be/miniscript/) inside Taproot leaves for descriptor wallets. (#27255)
...
- The usage of hybrid public keys in output descriptors has been removed. Hybrid
  ...
  (as specified in BIP380 and documented in doc/descriptors.md). Ilancoin Core would
...
- Bash completion files have been renamed from `ilancoin*.bash-completion` to
  `ilancoin*.bash`. This means completions can be automatically loaded on demand
