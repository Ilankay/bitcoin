# User-space, Statically Defined Tracing (USDT) for Ilancoin Core

Ilancoin Core includes statically defined tracepoints to allow for more
...
**Note**: Ilancoin Core uses temporary clones of the _main_ UTXO cache
...
## Adding tracepoints to Ilancoin Core
...
Multiple tools can list the available tracepoints in a `ilancoind` binary with
USDT support.

### GDB - GNU Project Debugger

To list probes in Ilancoin Core, use `info probes` in `gdb`:

```
$ gdb ./build/src/ilancoind
…
(gdb) info probes
Type Provider   Name             Where              Semaphore Object
stap net        inbound_message  0x000000000014419e 0x0000000000d29bd2 /build/src/ilancoind
stap net        outbound_message 0x0000000000107c05 0x0000000000d29bd0 /build/src/ilancoind
stap validation block_connected  0x00000000002fb10c 0x0000000000d29bd8 /build/src/ilancoind
…
```

### With `readelf`

The `readelf` tool can be used to display the USDT tracepoints in Ilancoin Core.
Look for the notes with the description `NT_STAPSDT`.

```
$ readelf -n ./build/src/ilancoind | grep NT_STAPSDT -A 4 -B 2
...
```
$ tplist -l ./build/src/ilancoind -v
