...
Ilancoin Core uses [Doxygen](https://www.doxygen.nl/) to generate its official documentation.
...
configuration options and toggled while ilancoind is running with the `logging`
RPC.  For instance, launching ilancoind with `-debug` or `-debug=1` will turn on
...
i.e. `CPPFLAGS="-DDEBUG_LOCKCONTENTION"`, then build and run ilancoind.

You can then use the `-debug=lock` configuration option at ilancoind startup or
`ilancoin-cli logging '["lock"]'` at runtime to turn on lock contention logging.
It can be toggled off again with `ilancoin-cli logging [] '["lock"]'`.
...
```shell
$ valgrind --suppressions=contrib/valgrind.supp build/src/test/test_ilancoin
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all build/src/test/test_ilancoin --log_level=test_suite
$ valgrind -v --leak-check=full build/src/ilancoind -printtoconsole
...
# A coverage report will now be accessible at `./build/test_ilancoin.coverage/index.html`,
...
- [Init load (`b-initload`)](https://doxygen.ilancoincore.org/namespacenode.html#ab4305679079866f0f420f7dbf278381d)
...
- [CCheckQueue::Loop (`b-scriptch.x`)](https://doxygen.ilancoincore.org/class_c_check_queue.html#a6e7fa51d3a25e7cb65446d4b50e6a987)
...
- [TorControlThread (`b-torcontrol`)](https://doxygen.ilancoincore.org/torcontrol_8cpp.html#a52a3efff23634500bb42c6474f306091)
...
- Net threads:
  ...
  - [ThreadI2PAcceptIncoming (`b-i2paccept`)](https://doxygen.ilancoincore.org/class_c_connman.html#a57787b4f9ac847d24065fbb0dd6e70f8)
...
Some of these are maintained by active developers of Ilancoin Core, in which case
...
a Ilancoin Core subtree, so that they can be integrated quickly. Cosmetic changes
...
- src/leveldb
  - Subtree at https://github.com/ilancoin-core/leveldb-subtree ; maintained by Core contributors.
...
- src/crc32c
  ...
  - Subtree at https://github.com/ilancoin-core/crc32c-subtree ; maintained by Core contributors.
  - Upstream at https://github.com/google/crc32c ; maintained by Google.

- src/secp256k1
  - Upstream at https://github.com/ilancoin-core/secp256k1/ ; maintained by Core contributors.

- src/crypto/ctaes
  - Upstream at https://github.com/ilancoin-core/ctaes ; maintained by Core contributors.
